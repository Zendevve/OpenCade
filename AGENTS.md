...
- **Protocol:** every WS message `Envelope {type:"signaling.offer", version:"1", request_id, timestamp, payload:{room_id,candidate}}` with `version` as **string** `"1"` matching `pub version: String` in `apps/server/src/main.rs:20` (and `packages/protocol::Envelope`). Server validates `version` then `type`, returns `{code:"unknown_message"}` for unknown `type`; forward-compatible bump via `"2"` handler.
